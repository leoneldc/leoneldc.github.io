...
button.onclick = async function() {
    ...
    fetch(`https://pokeapi.co/api/v2/pokemon/${url.toLowerCase()}`)
      ...
      .then(function (pokedata) {
        ...
        let randomVida = Math.floor(
          Math.random() * pokedata.stats[0].base_stat + 1
        );
        let randomAtaque = Math.floor(
          Math.random() * pokedata.stats[1].base_stat + 1
        );
        let randomDefensa = Math.floor(
          Math.random() * pokedata.stats[2].base_stat + 1
        );
        let randomVelocidad = Math.floor(
          Math.random() * pokedata.stats[3].base_stat + 1
        );
        let randomAtaqueEspecial = Math.floor(
          Math.random() * pokedata.stats[4].base_stat + 1
        );
        let randomDefensaEspecial = Math.floor(
          Math.random() * pokedata.stats[5].base_stat + 1
        );
